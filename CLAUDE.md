...
| **RetainedEarnings fallback**           | **RESOLVED** — fallback dropped. The full identity `Equity − CommonStock − APIC + TreasuryStock − AOCI` is the only correct reconstruction, but AlphaVantage does not return `accumulatedOtherComprehensiveIncome`, so it is uncomputable. `resolveRetainedEarnings` now returns the direct value or `null`. | When `retainedEarnings` is absent, `null` propagates and the Altman `.require("retainedEarnings")` guard flags `MISSING_DATA` — better than scoring T2 on the partial (CS+APIC-only) value, which mis-attributed treasury/AOCI by tens of billions for buyback-heavy issuers (AAPL). |
...
5. ~~**ICR no `OPERATING_LOSS` / `NO_DEBT` flagging**~~ — **FIXED** — `InterestCoverageRatio` now carries a `status` field (`COVERED` / `NO_DEBT` / `OPERATING_LOSS`). Both flags are `Success` (not `Failure`) so they never enter `calculationErrors` and do not push reports to `MISSING_DATA`. Detection is based solely on ICR inputs: EBIT sign + presence of `interestExpense` (verified live: MNST lease interest fires COVERED, not NO_DEBT). Flyway V10 adds `interest_coverage_status VARCHAR(20)` column.
