...
import static com.stock.screener.collector.adapter.out.web.yhfinance.YhFinanceClientMapper.toCommand;
...
class YhFinanceGateway implements YahooFinanceClient {
    ...
    private static void validateClientResponse(String symbol, QuoteSummaryResponse response) {
        if (response == null || response.quoteSummary() == null) {
            throw new ClientException("Null response from YH Finance API for symbol: %s, api repose: ", symbol,
                    response);
        } else if (response.quoteSummary().error() != null) {
            throw new ClientException("Error fetching data for symbol: %s, error: %s", symbol,
                    response.quoteSummary().error());
        }

        var results = response.quoteSummary().result();
        if (results == null || results.isEmpty()) {
            throw new ClientException("Null response from YH Finance API for symbol: %s, api response: %s", symbol,
                    response);
        }
    }
    ...
    private void persistLog(String ticker, Object response) {
        try {
            ...
            log.error("Failed to serialize YH Finance response for ticker={}, function={}", ticker,
                    YhFinanceGateway.DEFAULT_MODULES, e);
        }
    }
}
